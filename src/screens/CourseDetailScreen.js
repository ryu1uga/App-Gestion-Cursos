...
import {
  ...
  minutesOf, weeklyMinutes, isVirtual, fmtDuration,
} from '../lib/classes.js'
...
export default function CourseDetailScreen({ course, onBack }) {
  ...
  // nueva de este curso. Es la misma hoja que usa la pestaña Horario.
    ...
    <NestableScrollContainer contentContainerStyle={styles.container}>
      ...
      <Card style={{ marginBottom: 12 }}>
        ...
        {sessions.map((s) => {
          ...
          return (
            <Pressable key={s.id} style={styles.classRow} onPress={() => setHoja({ session: s })}>
        ...
        <Pressable style={styles.addEval} onPress={() => setHoja({ session: null })}>
      ...
      {hoja && (
        <ScheduleSheet
          key={hoja.session?.id ?? 'nueva'}
          lockCourseId={course.id}
          edit={hoja.session ? { kind: 'clase', courseId: course.id, session: hoja.session } : null}
